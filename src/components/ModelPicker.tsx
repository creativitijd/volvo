...
function iconSrc(model: string) {
  const slug =
    model === "V60 Cross Country" ? "v60cc" : model === "V90 Cross Country" ? "v90cc" : model.toLowerCase().replace(/[^a-z0-9]/g, "");
  return `/models/icon-${slug}.png`;
}

function byModel(a: ModelSummary, b: ModelSummary) {
  const num = (m: string) => Number(m.match(/\d+/)?.[0] ?? 0);
  const prefix = (m: string) => SERIES_RANK[m.match(/^[A-Z]+/)?.[0] ?? ""] ?? 9;
  return num(a.model) - num(b.model) || prefix(a.model) - prefix(b.model) || a.model.localeCompare(b.model, "nl");
}
...
export function ModelPicker({
  ...
  const sorted = [...models].sort(byModel);
  return (
      ...
      {sorted.map((m) => {
        ...
        return (
          ...
          >
            ...
            <img src={iconSrc(m.model)} alt="" className="mx-auto mt-2 h-[78px] w-[76%] object-contain" />
